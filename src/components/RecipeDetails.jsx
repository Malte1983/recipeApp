...
import { FaPencilAlt, FaTrash, FaHeart, FaHeartBroken } from 'react-icons/fa';
import { scaleIngredients, formatAmount } from './RecipeUtilities';
import { deleteRecipe, addFavorite, removeFavorite } from './FirebaseUtilities';
...
const RecipeDetails = ({ recipe, onBack, onEdit, isFavorite, user }) => {
    ...
    return (
        <div className='bg-white rounded-lg shadow-md p-4'>
            ...
            {recipe.imageUrl && (
                <img src={recipe.imageUrl} alt={recipe.title} className='w-full h-64 object-cover mb-4 rounded-lg' />
            )}
            <p className='mb-2'>{recipe.description}</p>
            <div className='mb-4'>
                <h4 className='font-semibold'>Zutaten:</h4>
                <ul>
                    {scaleIngredients(recipe.ingredients, 4, 4).map((ingredient, index) => (
                        <li key={index}>{formatAmount(ingredient.amount)} {ingredient.unit} {ingredient.name}</li>
                    ))}
                </ul>
            </div>
            <div className='mb-4'>
            ...
            <div className='flex justify-between'>
                ...
                {user && (
                    <div>
                        ...
                        <button onClick={() => handleDelete(recipe.id)} className='bg-red-500 text-white p-2 rounded mr-2'>
                            <FaTrash />
                        </button>
                        <button onClick={toggleFavorite} className={`bg-${isFavorite(recipe.id) ? 'red' : 'blue'}-500 text-white p-2 rounded`}>
